...
按上图所示，使用两个USB-to-UART 转接板分别连接 UART0 和 UART1，UART0 作为串口指令输入和查看打印信息端口，UART1 作为连接仪器测试端口。
...
.. note::
    ...
    - ESP 提供的 DTM 固件默认以 GPIO4 和 GPIO5 作为 UART1 测试管脚，如需更换可在 UART0 通过串口指令调整。
